...
이 문서는 `.js` 탄막 파일 하나를 어떻게 쓰는지 **전부** 설명한다. 길다.

> 처음이라면 이 문서 말고 **[치트시트 한 장](cheatsheet.md)**부터 보고,
> [`src/patterns/template.js`](../src/patterns/template.js)를 복사해서 고치는 게 빠르다.
> 편집기 자동완성과 `engine.stage.help()` 쓰는 법도 치트시트에 있다.

엔진 쪽 구조는 [main.md](main.md)에 있고, 여기서는 **패턴 작성자가 알아야 할 것만** 다룬다.
...
| `facing` | 각 탄의 방향: `'out'`(중심 반대) / `'in'` / `'aim'` / `'along'`(접선) / `'normal'`(변의 수직, 바깥) / 숫자(절대각) |
| `keepShape` | `true`면 먼 탄일수록 빠르게 나가서 **배치가 모양 그대로 확대**된다 |
...
같은 다각형이라도 `facing`/`keepShape`에 따라 결과가 전혀 다르다.

| 설정 | 결과 |
|---|---|
| `facing: 'out'` | 중심에서 방사상. 시간이 갈수록 **모서리가 뭉개져 원에 가까워진다** |
| `facing: 'out', keepShape: true` | **다각형이 그대로 확대**된다 (먼 탄일수록 빠름) |
| `facing: 'normal'` | 변이 길이를 유지한 채 밖으로 밀려난다. **모서리에 틈**이 벌어진다 |

정육각형(외접/내접 비 1.155)으로 80프레임 뒤를 재보면
`out`은 1.03(거의 원), `keepShape`는 1.155(그대로), `normal`은 1.008이 된다.
