export default {
    name: '호부「세계의 부레옥잠」 (LUNATIC)',

    clear: 'boss',
    hp: 2800,

    thresholds: [0.1, 0.85],
    difficulty: { density: 6, speed: 8, special: 6 },
...
function* ph2_shoot_1(s) {
    let iter = 0;
    while (true) {
        let angle = s.deg(Math.sin((iter / 180) * 3.14) * 45 + 90);
...
function* ph2_shoot_2(s) {
    yield 360;
    while (true) {
        let st = s.rand(-480, -420);
        let angle = s.pick([s.deg(75), s.deg(90), s.deg(105)]);
        for (let i = st; i < 450; i += 45) {
            s.fire({
                x: i,
                y: -240,
                speed: 2,
